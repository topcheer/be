...
public class RightsManagePanel extends VLayout {
	...
	protected static final DataBaseRPCAsync dbService = BrightEdu
			.createDataBaseRPC();
	...
	SectionStackSection selectionSection = new SectionStackSection(
			"功能列表 -- 要将某个功能加到选中的类别，拖动到左边的列表中");
	...
	// TransferImgButton arrowImg = new
	...
	protected void removeCategory() {
		...
		dbService.deleteRightsCatetoryFunctions(categoryItem.getValue()
		...
		dbService.getRightsCategory(new AsyncCallback<List<RightsCategory>>() {
			...
			public void onSuccess(List<RightsCategory> result) {
				...
				for (RightsCategory c : result) {
					list.put(c.getCategory_id(), "[ " + c.getCategory_id()
							+ " ] " + c.getCategory_name());
				}
...

	{
		...
		dbService.getRightsCategoryFunction(category_id,
				new AsyncCallback<List<RightsCategoryFunctionKey>>() {
					...
					public void onSuccess(List<RightsCategoryFunctionKey> result) {
						...
						for (RightsCategoryFunctionKey c : result) {
							Record rec = new Record();
							rec.setAttribute("funcId", c.getFunction_id());
							rec.setAttribute("funcName",
									funlist.get(c.getFunction_id()));
							list.add(rec);
						}
	...
	protected void saveMe() {
		...
		{
			dbService.deleteRightsCatetoryFunctions(categoryId,
					new AsyncCallback<Boolean>() {
						...
						public void onSuccess(Boolean result) {
							...
							BrightEdu
									.showTip("没有啥东西好存的,开玩笑? 我顺便把所有的组合全删了，你没意见吧？");
						}
					});
